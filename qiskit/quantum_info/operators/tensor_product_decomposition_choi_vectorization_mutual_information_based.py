...
# --- MI-based candidate generation (drop-in helpers) -------------------------
def _unitary_to_choi_state_tensor(op_local: np.ndarray) -> np.ndarray:
    """Return the local Choi 'operator state' |U> grouped per qubit as an
    n-party pure tensor with local dim=4. Axes are per-qubit pairs (out_i, in_i).

    Input:
        op_local: (2**m, 2**m) matrix in little-endian basis (qubit 0 is LSB).
    Output:
        psi_grouped: ndarray with shape (4,)*m representing a pure state.
    """
    dim, dim2 = op_local.shape
    if dim != dim2:
        raise QiskitError("Local operator must be square.")
    m = int(round(np.log2(dim)))
    if 2**m != dim:
        raise QiskitError("Local operator dimension must be a power of 2.")
    # Build the interleaved tensor psi[out0,in0,out1,in1,...] explicitly
    psi = np.zeros([2] * (2 * m), dtype=complex)
    for row in range(dim):
        out_bits = [(row >> k) & 1 for k in range(m)]  # LSB first
        for col in range(dim):
            in_bits = [(col >> k) & 1 for k in range(m)]
            idx = []
            for k in range(m):
                idx.extend([out_bits[k], in_bits[k]])
            psi[tuple(idx)] = op_local[row, col] / np.sqrt(dim)
    # Group per qubit: each (out_i, in_i) -> a single 4-level leg
    return psi.reshape([4] * m)


def _reduced_density_from_pure(psi_grouped: np.ndarray, subset: tuple[int, ...]) -> np.ndarray:
    """Partial trace over complement for a pure state tensor psi_grouped (shape (4,)*m)."""
    m = psi_grouped.ndim
    subset = tuple(sorted(subset))
    env = tuple(i for i in range(m) if i not in subset)
    psi_perm = np.transpose(psi_grouped, axes=subset + env)
    dS = 4 ** len(subset) if subset else 1
    dE = 4 ** len(env) if env else 1
    mat = psi_perm.reshape(dS, dE)
    return mat @ mat.conj().T  # Tr_env |psi><psi|


def _entropy_base2(rho: np.ndarray, tol: float = 1e-12) -> float:
    """Von Neumann entropy in bits (base-2)."""
    evals = np.linalg.eigvalsh((rho + rho.conj().T) / 2)  # hermitize
    evals = np.clip(evals.real, 0.0, 1.0)
    evals = evals[evals > tol]
    if evals.size == 0:
        return 0.0
    return float(-np.sum(evals * (np.log(evals) / np.log(2.0))))


def _choi_mutual_info_matrix(op_local: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Compute the m x m MI matrix between qubit legs (out_i,in_i) for the local operator."""
    psi = _unitary_to_choi_state_tensor(op_local)  # shape (4,)*m
    m = psi.ndim
    # Single-site entropies
    S1 = np.zeros(m)
    for i in range(m):
        rho_i = _reduced_density_from_pure(psi, (i,))
        S1[i] = _entropy_base2(rho_i)
    # Pairwise MI
    M = np.zeros((m, m))
    for i in range(m):
        for j in range(i + 1, m):
            rho_ij = _reduced_density_from_pure(psi, (i, j))
            M[i, j] = S1[i] + S1[j] - _entropy_base2(rho_ij)
            M[j, i] = M[i, j]
    return M, S1


def _mi_connected_components(M: np.ndarray, eps: float = 1e-10) -> list[list[int]]:
    """Connected components where edges are MI > eps."""
    m = M.shape[0]
    seen = [False] * m
    comps: list[list[int]] = []
    for s in range(m):
...
def _candidate_bipartitions_from_mi(
    op_local: np.ndarray,
    qubits_local: tuple[int, ...],
    *,
    max_candidates: int | None,
    eps_mi: float = 1e-10,
    choi_qubit_cap: int = 10,
) -> list[tuple[int, ...]]:
    """Return a small list of promising LOCAL-index bipartitions S (1<=|S|<=floor(m/2)),
    using a Choi mutual-information graph. If the local size m is too large for a
    Choi build (m > choi_qubit_cap), return [] to let the caller fall back.
    """
    m = len(qubits_local)
    if m <= 1 or m > choi_qubit_cap:
        return []

    M, S1 = _choi_mutual_info_matrix(op_local)
    comps = _mi_connected_components(M, eps=eps_mi)

    cands: list[tuple[int, ...]] = []

    # (A) If MI graph has >1 components, propose each component as S (union-of-components
    #     splits will be discovered by recursion, so one component per split suffices).
    if len(comps) > 1:
        for comp in comps:
            if 1 <= len(comp) <= m // 2:
                cands.append(tuple(comp))
    else:
        # (B) Single component: propose a spectral (Fiedler) cut and a few simple seeds.
        # Graph Laplacian L = D - M
        deg = M.sum(axis=1)
        L = np.diag(deg) - M
        # Compute the Fiedler vector (2nd smallest eigenvector)
        evals, evecs = np.linalg.eigh(L)
        if evals.size >= 2:
            fiedler = evecs[:, 1]
            median = np.median(fiedler)
            left = [i for i, v in enumerate(fiedler) if v <= median]
            right = [i for i, v in enumerate(fiedler) if v > median]
            # choose the smaller side to obey |S|<=m//2 (ignore empty sets)
            for side in (left, right):
                if 1 <= len(side) <= m // 2:
                    cands.append(tuple(sorted(side)))
                    break  # one spectral candidate is enough

        # Also add: best singleton (smallest single-site entropy) and strongest pair
        i_best = int(np.argmin(S1))
        if 1 <= 1 <= m // 2:
            cands.append((i_best,))
        # strongest pair by MI weight
        if m >= 2:
            i_pair, j_pair = np.unravel_index(np.argmax(np.triu(M, 1)), M.shape)
            if 2 <= m // 2:
                cands.append(tuple(sorted((i_pair, j_pair))))

    # Deduplicate, keep size constraints, and cap the list
    uniq = []
    seen = set()
    for S in cands:
        if not (1 <= len(S) <= m // 2):
...
def _best_bipartition_candidates(
    op_local: np.ndarray,
    qubits_local: tuple[int, ...],
    *,
    max_partitions: int | None,
    ...
    """Pick the best bipartition from a *small* MI-guided candidate list.
    Falls back to the exhaustive scanner if candidates are empty.
    Returns: (S_global, CutInfo, A_factor, B_factor)
    """
    # Build local-index candidates
    cands_local = _candidate_bipartitions_from_mi(
        op_local,
        qubits_local,
        max_candidates=max_partitions if max_partitions is not None else 32,
        eps_mi=eps_mi,
        choi_qubit_cap=choi_qubit_cap,
    )
    if not cands_local:
        print("fell")
        # Fallback: use the existing exhaustive search (honors max_partitions)
        return _best_bipartition_all_sizes(op_local, qubits_local, max_partitions=max_partitions)

    best = (None, None, None, None, None)  # info, a, b, S_glob, Sc_glob
    tested = 0
    for s_loc in cands_local:
        if max_partitions is not None and tested >= max_partitions:
            break
        tested += 1
        res = operator_schmidt_decomposition(
            op_local, qargs=s_loc, k=1, return_reconstruction=False
        )
        svals = res["singular_values"]
        tail = float(np.sqrt(np.sum(svals[1:] ** 2))) if svals.size > 1 else 0.0
        s_glob = tuple(qubits_local[i] for i in s_loc)
        sc_glob = tuple(q for q in qubits_local if q not in s_glob)
        info = CutInfo(
            subset_s=tuple(sorted(s_glob)),
            subset_sc=tuple(sorted(sc_glob)),
            singular_values=svals,
            frobenius_error=tail,
            relative_frobenius_error=float(tail / (np.linalg.norm(svals) + 1e-16)),
            kept_terms=1,
        )
        a_factor = res["A_factors"][0]
        b_factor = res["B_factors"][0]
        if best[0] is None or tail < best[0].frobenius_error:
            best = (info, a_factor, b_factor, s_glob, sc_glob)

    if best[0] is None:
        # Safety fallback
        return _best_bipartition_all_sizes(op_local, qubits_local, max_partitions=max_partitions)

    info, a_factor, b_factor, S_glob, _ = best
    return S_glob, info, a_factor, b_factor


# --- end MI-based candidate generation ---------------------------------------
...
def _factorize_recursive(
    ...
    *,
    ...
    s_global, info, a_factor, b_factor = _best_bipartition_candidates(
        op_local,
        qubits_local,
        max_partitions=max_partitions,
        eps_mi=1e-10,
